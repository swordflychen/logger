...
#ifdef OPEN_DEBUG  /*only for format check*/
#include <stdio.h>
#define debug(level, format, ...) {\
    fprintf(stderr, "LVL: %10s| FILE: %20s | FUN: %20s | LINE: %4d | MSG: ", \
            level, __FILE__, __FUNCTION__, __LINE__); \
    fprintf(stderr, format, ##__VA_ARGS__); \
    fprintf(stderr, "\n"); \
}

#define logEmerg(format, ...)   debug("emerg", format,  ##__VA_ARGS__)
#define logCrit(format, ...)    debug("crit", format,  ##__VA_ARGS__)
#define logAlert(format, ...)   debug("alert", format,  ##__VA_ARGS__)
#define logError(format, ...)   debug("error", format,  ##__VA_ARGS__)
#define logWarning(format, ...) debug("warning", format,  ##__VA_ARGS__)
#define logNotice(format, ...)  debug("notice", format,  ##__VA_ARGS__)
#define logInfo(format, ...)    debug("info", format,  ##__VA_ARGS__)
#define logDebug(format, ...)   debug("debug", format,  ##__VA_ARGS__)
